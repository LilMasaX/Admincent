...
export async function GET(
  ...
  const tpl = await sb
    .from("templates")
    .select(
      "id, name, kind, storage_path, fields, page_sizes, logo_position, tipo, created_at, owner_id",
    )
    .eq("id", id)
    .eq("tipo", "asistencia")
    .maybeSingle();
...
export async function DELETE(
  ...
  const tpl = await sb
    .from("templates")
    .select("storage_path, owner_id, tipo")
    .eq("id", id)
    .maybeSingle();
  if (tpl.error || !tpl.data) return NextResponse.json({ error: "not found" }, { status: 404 });
  ...
  const del = await sb.from("templates").delete().eq("id", id);
